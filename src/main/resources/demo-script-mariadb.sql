...
CREATE TABLE `formateur` (
	...
	`id_photo` INT NULL, 
	PRIMARY KEY (`email`) USING BTREE, CONSTRAINT `FK_FOR_COURS` FOREIGN KEY (`id_cours_principal`) REFERENCES `cours_eni` (`id`) ON
UPDATE NO ACTION ON
DELETE NO ACTION, CONSTRAINT `FK_FOR_FICHIER` FOREIGN KEY (`id_photo`) REFERENCES `fichier` (`id`) ON
UPDATE NO ACTION ON
DELETE NO ACTION
);
...
CREATE TABLE `cours_formateur` (
	...
	`id_cours` INT(11) NOT NULL, PRIMARY KEY (`email_formateur`, `id_cours`) USING BTREE, INDEX `fk_COURS_ENI_cours_formateur` (`id_cours`) USING BTREE, CONSTRAINT `fk_COURS_ENI_cours_formateur` FOREIGN KEY (`id_cours`) REFERENCES `cours_eni` (`id`) ON
UPDATE NO ACTION ON
DELETE NO ACTION, CONSTRAINT `fk_FORMATEURS_cours_formateur` FOREIGN KEY (`email_formateur`) REFERENCES `formateur` (`email`) ON
UPDATE NO ACTION ON
DELETE NO ACTION
);
